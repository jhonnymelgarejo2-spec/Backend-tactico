...
try:
    from core.final_outcome_engine import (
        evaluar_desenlace_final,
        aplicar_desenlace_final_a_senal,
    )
except Exception:
    evaluar_desenlace_final = None
    aplicar_desenlace_final_a_senal = None
...
def generar_senal_fallback(datos: Dict) -> Dict:
    ...
    if minuto >= 70 and xg >= 1.3:
        market = "NEXT_GOAL"
        apuesta = "Habrá gol (próximo)"
        reason = "Fallback tardío con suficiente actividad ofensiva"
    elif xg >= 1.2:
        market = "OVER_MATCH_DYNAMIC"
        apuesta = "Más goles en el partido"
        reason = "Fallback por presión ofensiva aceptable"
    else:
        market = "RESULT_HOLDS_NEXT_15"
        apuesta = "Se mantiene el resultado próximos 15 min"
        reason = "Fallback por baja presión ofensiva"
    ...
    return {
        ...
        "ganador_probable": "LOCAL" if ml > mv else "VISITANTE" if mv > ml else "EMPATE",
        "doble_oportunidad_probable": "LOCAL_O_EMPATE" if ml >= mv else "VISITANTE_O_EMPATE",
        "total_goles_estimado": ml + mv + 1,
        "linea_goles_probable": "OVER_2_5" if (ml + mv) >= 2 else "UNDER_2_5",
        "over_under_probable": "OVER 2.5" if (ml + mv) >= 2 else "UNDER 2.5",
...
# =========================================================
def _build_input_signal(partido: Dict, senal: Dict) -> Dict:
    return {
...
# =========================================================
# FILTRO MAESTRO FLEXIBLE
# =========================================================
def _master_gate(senal_final: Dict) -> bool:
    minute = _safe_int(senal_final.get("minute"), 0)
    confidence = _safe_float(senal_final.get("confidence"), 0.0)
    value = _safe_float(senal_final.get("value"), 0.0)
    risk_score = _safe_float(senal_final.get("risk_score"), 10.0)
    tactical_score = _safe_float(senal_final.get("tactical_score"), 0.0)
    signal_score = _safe_float(senal_final.get("signal_score"), 0.0)
    final_state = _safe_upper(senal_final.get("final_outcome_state", ""))
    final_decision = _safe_upper(senal_final.get("final_decision_recommended", ""))
    hold_score = _safe_float(senal_final.get("hold_result_score", 0.0))
    next_goal_score = _safe_float(senal_final.get("next_goal_score", 0.0))
    over_score = _safe_float(senal_final.get("over_final_score", 0.0))
    under_score = _safe_float(senal_final.get("under_final_score", 0.0))

    best_outcome = max(hold_score, next_goal_score, over_score, under_score)
...
def procesar_partido(partido: Dict) -> Optional[Dict]:
    ...
    # 2. NORMALIZAR
    # =========================================
    senal_final = _build_input_signal(partido, senal)
    ...
    # =========================================
    # 4.1 FINAL OUTCOME ENGINE
    # =========================================
    if evaluar_desenlace_final and aplicar_desenlace_final_a_senal:
        try:
            final_data = evaluar_desenlace_final(partido, senal_final)
            senal_final = aplicar_desenlace_final_a_senal(senal_final, final_data)
        except Exception as e:
            print(f"[PIPELINE] ERROR FINAL OUTCOME -> {e}")
    ...
    final_decision = _safe_upper(senal_final.get("final_decision_recommended", ""))
    final_market = _safe_upper(senal_final.get("final_market_recommended", ""))
    final_reason = _safe_text(senal_final.get("final_market_reason", ""))

    if final_decision and final_decision != "NO_APOSTAR" and final_market:
        senal_final["market"] = final_market
        senal_final["selection"] = senal_final.get("final_selection_recommended", senal_final.get("selection"))
        senal_final["reason"] = final_reason or senal_final.get("reason", "")
        senal_final["recomendacion_final"] = final_decision
        senal_final["ai_recommendation"] = final_decision
    ...
    print(
        ...
        f"final_state={senal_final.get('final_outcome_state')}"
    )
