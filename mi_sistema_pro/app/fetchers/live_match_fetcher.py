...
def _build_headers() -> Dict[str, str]:
    ...
    return {
        ...
        "User-Agent": "MI_SISTEMA_PRO/1.0",
    }


def _build_url() -> str:
    return settings.FOOTBALL_API_URL or DEFAULT_API_URL
...
def _fetch_fixture_statistics(fixture_id: int, headers: Dict[str, str]) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"[FETCHER] Error obteniendo estadísticas fixture {fixture_id}: {e}")
        return {}
...
def _normalizar_fixture(item: Dict[str, Any], headers: Dict[str, str], fetched_at: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "pais": safe_text(league.get("country"), "Mundo"),
        "minuto": minuto,
...
def obtener_partidos_en_vivo() -> List[Dict[str, Any]]:
    try:
        ...
        if not isinstance(fixtures, list):
            print("[FETCHER] respuesta inválida de API-Football")
            return []
        ...
        for item in fixtures:
            try:
                ...
                print(f"[FETCHER] Error normalizando fixture: {e}")

        print(f"[FETCHER] partidos live normalizados: {len(resultados)}")
        return resultados

    except Exception as e:
        print(f"[FETCHER] Error general API-Football: {e}")
        return []
